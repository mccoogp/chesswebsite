...
class Super_king():
    ...
    def promotion(self, pos):
        self.board.board[pos[0]][pos[1]] = piece.Queen(False)
    ...
    def move(self, start, to):
        ...
        if self.turn and not target_piece.color:
            #print("That's not your piece to move")
            return
        if not self.turn and target_piece.color:
            return
        ...
        if target_piece.is_valid_move(self.board, start, to):
            ...
            i = 0
            while i < 8:
                if self.turn and self.board.board[0][i] != None and \
                    self.board.board[0][i].name == 'P':
                    self.board.board[0][i] = piece.Queen(False)
                    break
                elif self.turn and self.board.board[7][i] != None and \
                    self.board.board[7][i].name == 'P':
                    self.board.board[7][i] = piece.Queen(True)
                    break
                i += 1
...
class Forward():
    ...
    def promotion(self, pos):
        self.board.board[pos[0]][pos[1]] = piece.Queen(True)
    ...
    def move(self, start, to):
        ...
        precondition: `start` and `to` are valid positions on the board
        """
        if self.turn and start[0] >= to[0]:
            return
        if not self.turn and start[0] <= to[0]:
            return
        if self.board.board[start[0]][start[1]] == None:
            #print("There is no piece to move at the start place")
            return
        ...
        if target_piece.is_valid_move(self.board, start, to):
            ...
            while i < 8:
                if self.turn and self.board.board[0][i] != None and \
                    self.board.board[0][i].name == 'P':
                    self.board.board[0][i] = piece.Queen(True)
                    break
                elif not self.turn and self.board.board[7][i] != None and \
                    self.board.board[7][i].name == 'P':
                    self.promotion((7, i))
                    self.board.board[7][i] = piece.Queen(True)
                    break
